...
pygame.display.set_caption('Plotformir')
...
class Platform:
    ...
    def draw(self, surf,camera_x = 0):
        pygame.draw.rect(surf,(60,180,90),(self.rect - camera_x, self.rect.centery))
...
class Enemy:
    ...
    def update(self):
        ...
        if self.rect.left <= self.left_limit or self.rect.right >= self.right_limit:
            self.dir += -1
    def draw(self,surf,camera_x = 0):
        pygame.draw.rect(surf, (220,70,70), (self.rect.x - camera_x, self.rect.y,self.rect.x,self.rect.h))
...
class Player:
    ...
    def draw(self,surf,camera_x = 0):
        ...
        pygame.draw.rect(surf, (80,140,255), (self.rect.x - camera_x,self.rect.y - camera_x,self.rect.width,self.rect.height))
    ...
    class Game:
        ...
        def collect_coin(self):
            for c in self.coins:
                self.player.rect.colliderect(c.rect)
                self.score += 1
                self.coin.remove
        ...
        def run(self):
            ...
            while running:
                ...
                screen.blit(font.render(F"lives: " + str(self.player.lives), True, (0, 0, 0), (10, 40)))
                ...
                if self.game_over:
                    ...
                    screen.blit(t1.get_rect(center=(w // 2,h // 2 - 20)))
                    screen.blit(t2.get_rect(center=(w // 2,h // 2 + 25)))
